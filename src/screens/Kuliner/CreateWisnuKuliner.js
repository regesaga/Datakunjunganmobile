...
const CreateWisnuKuliner = ({ route }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    jumlah_laki_laki: [],
    jumlah_perempuan: [],
    wismannegara_id: [], // Terisi dengan ID kulinerwan mancanegara
    jml_wisman_laki: [],
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setFormData((prev) => ({
          ...prev,
          kuliner_id: response.data.data.kuliner_id,
          ...
          wismannegara_id: response.data.data.wismannegara.map((wisman) => wisman.id), // Set ID kulinerwan mancanegara
        }));
      })
      .catch((error) => {
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Tambah Kunjungan {apiData.kuliner}</Text>
...
export default CreateWisnuKuliner;
