...
const TabBarKuliner = ({ navigation }) => {
  ...
  const handleTabPress = async (tabIndex) => {
    ...
    if (tabIndex === 1) { // Index for "Kunjungan" tab
      if (token) {
        navigation.navigate('KunjunganKuliner', { token }); // Navigate to KunjunganKuliner with token
      } else {
        Alert.alert('Error', 'Token tidak ditemukan!');
      }
    } else if (tabIndex === 2) { // Index for "Tambah" tab
      if (token) {
        navigation.navigate('CreateWisnuKuliner', { token }); // Navigate to CreateWisnuKuliner with token
      } else {
        Alert.alert('Error', 'Token tidak ditemukan!');
      }
    } else {
