...
const EditWisnuKuliner = ({ route }) => {
  const { token, kulinerId, tanggalKunjungan } = route.params; // Get kuliner_id and tanggal_kunjungan from route params
  ...
  useEffect(() => {
    axios
      .get(`${URL}/api/v1/kunjungankuliner/edit/${kulinerId}/${tanggalKunjungan}`, {
        headers: {
  ...
  }, [token, kulinerId, tanggalKunjungan]);
  ...
  const handleSubmit = () => {
    const requestData = {
      kuliner_id: formData.kuliner_id,
    ...
    axios
    .put(`${URL}/api/v1/kunjungankuliner/update/${tanggalKunjungan}`, requestData, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
...
export default EditWisnuKuliner;
