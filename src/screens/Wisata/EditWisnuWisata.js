...
const EditWisnuWisata = ({ route }) => {
  const { token, wisataId, tanggalKunjungan } = route.params; // Get wisata_id and tanggal_kunjungan from route params
  ...
  useEffect(() => {
    axios
      .get(`${URL}/api/v1/kunjunganwisata/edit/${wisataId}/${tanggalKunjungan}`, {
        headers: {
  ...
  }, [token, wisataId, tanggalKunjungan]);
  ...
  const handleSubmit = () => {
    ...
    axios
    .put(`${URL}/api/v1/kunjunganwisata/update/${tanggalKunjungan}`, requestData, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
  ...
  />
        ...
        <Text style={styles.headerTitle}>Ubah Kunjungan {apiData.wisata}</Text>
      </View>
...
export default EditWisnuWisata;
