...
const KunjunganAdmin = ({ route }) => {
  ...
  const getDataKunjunganAdmin = async () => {
    setLoading(true);
    setRefresh(true);
    try {
      const response = await axios.get(`${URL}/api/v1/kunjunganadmin/getAll`, {
        headers: {
  ...
  const renderEventDetails = ({ item }) => (
    <View style={styles.cardContainer} key={item.tanggal_kunjungan}>
      <View style={styles.cardHeader}>
        <Text style={styles.dateText}>{formatDateIndonesia(item.tanggal_kunjungan)}</Text>
        <Text style={styles.namaadmin}>Jumlah Kunjungan {item.total_kunjungan}</Text>
      </View>
  ...
  return loading ? (
    ...
    <>
      ...
      <FlatList
        data={filteredEvents}
        keyExtractor={(item) => item.tanggal_kunjungan} // Menggunakan tanggal_kunjungan sebagai key
