...
const TabBarAkomodasi = ({ navigation }) => {
  ...
  const handleTabPress = async (tabIndex) => {
    ...
    if (tabIndex === 1) { // Index for "Kunjungan" tab
      if (token) {
        navigation.navigate('KunjunganAkomodasi', { token }); // Navigate to KunjunganAkomodasi with token
      } else {
        Alert.alert('Error', 'Token tidak ditemukan!');
      }
    } else if (tabIndex === 2) { // Index for "Tambah" tab
      if (token) {
        navigation.navigate('CreateWisnuAkomodasi', { token }); // Navigate to CreateWisnuAkomodasi with token
      } else {
        Alert.alert('Error', 'Token tidak ditemukan!');
      }
    } else {
